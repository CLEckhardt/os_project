...
impl SchemeMut for ZeroScheme {
    ...
    fn fstat(&mut self, _: usize, stat: &mut syscall::Stat) -> Result<usize> {
        stat.st_mode = 0o666 | MODE_CHR; // Access mode | type of file (special char file here
        stat.st_size = 0; // File size
        stat.st_blocks = 0; // Number of blocks 
        stat.st_blksize = 4096; // IO block size
        stat.st_nlink = 1; // Number of links
